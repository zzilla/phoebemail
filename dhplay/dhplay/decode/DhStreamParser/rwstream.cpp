...
int RwStream::CheckIfIFrame(unsigned char* buf, int signpos)
{
	...
	tmp_IFrameHeadLenth = 0 ; //fix bug：流情况下，I帧获取不正确
	...
	if (bfindIFrame)
	{
		...
		while (tmp_code != 0x000001B6)
		{
			...
			if (tmp_code == 0x0120)
			{
				...
				int nRate = 0;
				int nRet = Mpeg4_Analyse(tmp_buf-8,100,&tmp_frameinfo->nWidth,&tmp_frameinfo->nHeight,&nRate);
...
L:
	if (m_FrameInfo)
	{
		...
		else if (m_FrameInfo->nType == DH_FRAME_TYPE_AUDIO)
		{
			...
			{
				m_FrameInfo->nEncodeType = 31;//#define AVI_AUDIO   31
