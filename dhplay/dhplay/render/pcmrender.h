...
class CPcmRender : public CAudioRender
{
	struct PcmBuffer {
		...
		DWORD Prepare(HWAVEOUT hOut, unsigned int count, unsigned int *uiWrittenLen)
		{
			Release();
			...
			header.lpData = data;
			header.dwBufferLength = (count > BUFFER_SIZE)? BUFFER_SIZE : count;
			header.dwUser = (DWORD)this;

			*uiWrittenLen = header.dwBufferLength;
