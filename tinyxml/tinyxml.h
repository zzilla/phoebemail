...
**	Description	: 增加doc到xml字符串的转换
**
**	Modification	: 2006/6/10		jacko
**					add DocToString(), Printe(), SaveString()等函数
**
**					  2008/6/18     gaowei
**					为支持中文，作以下修改，此修改之后 {&，<，>，"，'}这5个特殊字符将不能使用
**					修改isspace()使用的参数，强制转换成unsigned char再使用
**					修改Printe()、Print()等使用PutString的地方，不使用PutString对name和value做一次拷贝，直接使用变量
**					
**					  2008/08/27	gaowei
**					重载DocToString(), Printe()函数，确保拷贝不越界
**  ************************************************************************
*/
...
#ifdef WIN32
typedef unsigned __int64	TiXmlUInt64;
#else
typedef unsigned long long	TiXmlUInt64;
#endif
...
/***********************add by jacko**************************/
//#ifdef _SAVESTRING
	virtual void Printe( char * xmlstr, int depth ) = 0 ;
	virtual int Printe( char * xmlstr, int totallen, int depth, int& len ) = 0 ;
//#endif
...
/********************************add by jacko****************************************/
//#ifdef _PUTSTRING
	virtual void Printe( char * xmlstr, int depth ) ;
	virtual int Printe( char * xmlstr, int totallen, int depth, int& len );
//#endif	//_PUTSTRING
...
public:
	...
	*/
	...
	void SetAttribute( const char * name, TiXmlUInt64 value );
...
/***************************************add by jacko****************/
	...
	//bool SaveFile(char * xmlstr) ;

//#ifdef _SAVESTRING

	bool DocToString(char * xmlstr) ;
	bool DocToString(char * xmlstr, int totallen, int& len) ;
	bool SaveString( char * xmlstr ) ;
	virtual void Printe( char * xmlstr, int depth ) ;
	virtual int Printe( char * xmlstr, int totallen, int depth, int& len );
