...
{ return (_istalnum(c)||c==_T('_')||c==_T(':')||c==_T('-')||c==_T('.')
	/* ---------- Modified by Ivan L. OLinS 2003-9-12 -------------- */
	// To support chinese
	|| c < 0
	/* ----------- End ------------- */
	 ); 
...
static int strcmpwild_impl(const TCHAR* src,const TCHAR* dst); //Forward declaration.
...
/* ------------------ added by Ivan L. OLinS -------------------- */
/*
 *	to support => unsigned long, unsigned int, unsigned short, unsigned char
 */
...
/* ------------------ added by Ivan L. OLinS -------------------- */
	//<summary>Set attribute to unsigned long.</summary>
	//<param name="rhs">Value unsigned long to set.</param>
	//<returns>Reference to xml_attribute.</returns>
	xml_attribute& operator=(unsigned long rhs)
	{
		TCHAR temp[32] = {0};
		_stprintf(temp,_T("%u"),rhs);
		value(temp);
		return *this;
	}

	//<summary>Set attribute to unsigned long.</summary>
	//<param name="rhs">Value unsigned int to set.</param>
	//<returns>Reference to xml_attribute.</returns>
	...
	//<summary>Set attribute to unsigned long.</summary>
	//<param name="rhs">Value unsigned short to set.</param>
	//<returns>Reference to xml_attribute.</returns>
	xml_attribute& operator=(unsigned short rhs)
	{
		TCHAR temp[32] = {0};
		_stprintf(temp,_T("%u"),rhs);
		value(temp);
	...
	//<summary>Set attribute to unsigned long.</summary>
	//<param name="rhs">Value unsigned char to set.</param>
	//<returns>Reference to xml_attribute.</returns>
	xml_attribute& operator=(unsigned char rhs)
	{
		TCHAR temp[32] = {0};
		_stprintf(temp,_T("%u"),rhs);
		value(temp);
...
public:
	...
	bool operator!=(const xml_node& r){ return (_root != r._root); } //True if this do not has the same internal xml_node_struct pointer value.
...
public:
	...
	bool parse(TCHAR* s,unsigned long optmsk = parse_noset)
	{
		if(!s) return false;
		clear(); //Free any allocated memory.
		if(strcatgrow(&_buffer, s))
		{
			_xmldoc = new_node(node_document); //Allocate a new root.
			_xmldoc->parent = _xmldoc; //Point to self.
			if(optmsk != parse_noset) _optmsk = optmsk;
			TCHAR* s = pug::parse(_buffer,_xmldoc,_growby,_optmsk); //Parse the input string.
			_strpos = s;
			return true;
		}
