	...
	: CDialog(CConfigAlarmMotion::IDD, pParent)
{
	...
	//}}AFX_DATA_INIT
	m_pDev = 0;
	memset(m_motionCFG, 0, MAX_VIDEO_IN_NUM*sizeof(MOTION_DETECT_CFG));
	m_bReady = FALSE;
	m_chlIdx = -1;
	m_weekIdx = -1;
	m_ptzchlIdx = -1;
...
void CConfigAlarmMotion::SetMotionCFG(MOTION_DETECT_CFG *cfg)
{
	if (!cfg)
	{
		return;
	...
	memcpy(m_motionCFG, cfg, MAX_VIDEO_IN_NUM*sizeof(MOTION_DETECT_CFG));

	DWORD dwActMask = m_motionCFG[0].struHandle.dwActionMask;
	//sentence if some functions enabled
	GetDlgItem(IDC_CHECK_UPLOAD)->EnableWindow((dwActMask & ALARM_UPLOAD)?TRUE:FALSE);
	GetDlgItem(IDC_CHECK_LOCALTIP)->EnableWindow((dwActMask & ALARM_TIP)?TRUE:FALSE);
	GetDlgItem(IDC_CHECK_LOCALTOUR)->EnableWindow((dwActMask & ALARM_TOUR)?TRUE:FALSE);
	GetDlgItem(IDC_CHECK_SENDMAIL)->EnableWindow((dwActMask & ALARM_MAIL)?TRUE:FALSE);
	
	//sentence if ptz enabled
	BOOL bPTZEn = (dwActMask & ALARM_PTZ)?TRUE:FALSE;
	...
	BOOL bRecEn = (dwActMask & ALARM_RECORD)?TRUE:FALSE;
	...
	BOOL bOutEn = (dwActMask & ALARM_OUT)?TRUE:FALSE;
...
void CConfigAlarmMotion::OnSelchangeComboVideochl() 
{
	...
	if (m_weekIdx >= 0)
	{
		...
		for (i = 0; i < 6; i++)
		{
			...
			m_motionCFG[m_chlIdx].stSect[m_weekIdx][i].iHourEnd = GetDlgItemInt(IDC_TIME_18+secIdx);
			secIdx++;
	...
	m_motionCFG[m_chlIdx].struHandle.dwActionFlag |= ((CButton*)GetDlgItem(IDC_CHECK_UPLOAD))->GetCheck()?ALARM_UPLOAD:0;
	m_motionCFG[m_chlIdx].struHandle.dwActionFlag |= ((CButton*)GetDlgItem(IDC_CHECK_LOCALTIP))->GetCheck()?ALARM_TIP:0;
	m_motionCFG[m_chlIdx].struHandle.dwActionFlag |= ((CButton*)GetDlgItem(IDC_CHECK_LOCALTOUR))->GetCheck()?ALARM_TOUR:0;
	m_motionCFG[m_chlIdx].struHandle.dwActionFlag |= ((CButton*)GetDlgItem(IDC_CHECK_SENDMAIL))->GetCheck()?ALARM_MAIL:0;
	m_motionCFG[m_chlIdx].struHandle.dwActionFlag |= ((CButton*)GetDlgItem(IDC_CHECK_PTZ))->GetCheck()?ALARM_PTZ:0;
	m_motionCFG[m_chlIdx].struHandle.dwActionFlag |= ((CButton*)GetDlgItem(IDC_CHECK_RECORD))->GetCheck()?ALARM_RECORD:0;
	m_motionCFG[m_chlIdx].struHandle.dwActionFlag |= ((CButton*)GetDlgItem(IDC_CHECK_OUTPUT))->GetCheck()?ALARM_OUT:0;
...
skipsave:
	...
	((CButton*)GetDlgItem(IDC_CHECK_UPLOAD))->SetCheck((m_motionCFG[chlIdx].struHandle.dwActionFlag&ALARM_UPLOAD)?1:0);
	((CButton*)GetDlgItem(IDC_CHECK_LOCALTIP))->SetCheck((m_motionCFG[chlIdx].struHandle.dwActionFlag&ALARM_TIP)?1:0);
	((CButton*)GetDlgItem(IDC_CHECK_LOCALTOUR))->SetCheck((m_motionCFG[chlIdx].struHandle.dwActionFlag&ALARM_TOUR)?1:0);
	((CButton*)GetDlgItem(IDC_CHECK_SENDMAIL))->SetCheck((m_motionCFG[chlIdx].struHandle.dwActionFlag&ALARM_MAIL)?1:0);
	((CButton*)GetDlgItem(IDC_CHECK_PTZ))->SetCheck((m_motionCFG[chlIdx].struHandle.dwActionFlag&ALARM_PTZ)?1:0);
	((CButton*)GetDlgItem(IDC_CHECK_RECORD))->SetCheck((m_motionCFG[chlIdx].struHandle.dwActionFlag&ALARM_RECORD)?1:0);
	((CButton*)GetDlgItem(IDC_CHECK_OUTPUT))->SetCheck((m_motionCFG[chlIdx].struHandle.dwActionFlag&ALARM_OUT)?1:0);
...
void CConfigAlarmMotion::OnSelchangeComboAlarmweek() 
{
	...
	for (int i = 0; i < 6; i++)
	{
		...
		SetDlgItemInt(IDC_TIME_18+timeCtrlIdx, m_motionCFG[m_chlIdx].stSect[weekIdx][i].iHourEnd);
		timeCtrlIdx++;
...
void CConfigAlarmMotion::OnBtnArea() 
{
	...
	{
		memcpy(m_motionCFG[m_chlIdx].byDetected, areaDlg.GetArea(), MOTION_ROW_NUM*MOTION_COL_NUM);
	}
}

void CConfigAlarmMotion::GetMotionCFG(MOTION_DETECT_CFG *cfg)
{
	...
	memcpy(cfg, m_motionCFG, MAX_ALARM_IN_NUM*sizeof(MOTION_DETECT_CFG));
}
