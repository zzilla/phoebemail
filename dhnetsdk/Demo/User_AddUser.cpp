...
void CUser_AddUser::SetEnvrmt(USER_MANAGE_INFO *info, DWORD gpID, DeviceNode *dev)
{
	m_user_info = info;
	m_gpID = gpID;
	m_dev = dev;
...
void CUser_AddUser::OnBtnOk() 
{
	...
	GetDlgItem(IDC_NAME_EDIT)->GetWindowText(urInfo.name, USER_NAME_LENGTH);
	GetDlgItem(IDC_PSW_EDIT)->GetWindowText(urInfo.passWord, USER_PSW_LENGTH);
	GetDlgItem(IDC_MEMO_EDIT)->GetWindowText(urInfo.memo, MEMO_LENGTH);
