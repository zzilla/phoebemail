	...
	: CDialog(CConfigComm::IDD, pParent)
{
	...
	//}}AFX_DATA_INIT
	m_pDev = 0;
	memset(&m_comCFG, 0, sizeof(DEV_COMM_CFG));
	m_bInited = FALSE;
...
void CConfigComm::GetConfig()
{
	...
	DWORD retlen = 0;
	BOOL bRet = CLIENT_GetDevConfig(m_pDev->LoginID, DEV_COMMCFG, 0,
								&m_comCFG, sizeof(DEV_COMM_CFG), &retlen, CONFIG_WAITTIME);
	int aaa = sizeof(DEV_COMM_CFG);
	if (!bRet || retlen != sizeof(DEV_COMM_CFG))
	{
...
BOOL CConfigComm::OnInitDialog() 
{
	...
	DWORD retlen = 0;
	DEV_SYSTEM_ATTR_CFG sysCFG = {0};
	BOOL bRet = CLIENT_GetDevConfig(m_pDev->LoginID, DEV_DEVICECFG, 0,
								&sysCFG, sizeof(DEV_SYSTEM_ATTR_CFG), &retlen, CONFIG_WAITTIME);
...
void CConfigComm::OnSelchangeCombo232com() 
{
	...
	if (Idx232 < 0 || Idx232 >= MAX_232_NUM) 
	{
		return;
	}
...
void CConfigComm::OnSelchangeCombo485com() 
{
	...
	if (Idx485 < 0 || Idx485 >= MAX_DECODER_NUM) 
	{
		return;
	}
...
void CConfigComm::OnApply() 
{
	...
	BOOL bRet = CLIENT_SetDevConfig(m_pDev->LoginID, DEV_COMMCFG, 0,
								&m_comCFG, 16*sizeof(DEV_COMM_CFG), CONFIG_WAITTIME);
	if (!bRet)
	{
		((CNetSDKDemoDlg*)AfxGetMainWnd())->LastError();
		return;
