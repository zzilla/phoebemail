...
BOOL CConfigAlarmMotionArea::OnInitDialog() 
{
	...
	int blckWid = (m_rect.Width()-MOTION_COL_NUM+1)/MOTION_COL_NUM;
	m_blckWid = blckWid;
	int blckHght = (m_rect.Height()-MOTION_ROW_NUM+1)/MOTION_ROW_NUM;
	m_blckHght = blckHght;
	for (int i = 0; i < MOTION_ROW_NUM; i++)
	{
		for (int j = 0; j < MOTION_COL_NUM; j++)
		{
			m_block[i][j].Create(
			NULL, 
			NULL, 
...
void CConfigAlarmMotionArea::SetArea(BYTE *area)
{
	...
	memcpy(m_myArea, area, MOTION_COL_NUM*MOTION_ROW_NUM);
