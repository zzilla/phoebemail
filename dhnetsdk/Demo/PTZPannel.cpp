...
BEGIN_MESSAGE_MAP(CPTZPannel, CDialog)
	...
	ON_BN_CLICKED(IDC_YWPTZCTRL, OnYwptzctrl)
...
BOOL CPTZPannel::OnInitDialog() 
{
	...
	//云台控制控件的命令定义设置
	m_ptz_up.SetButtonCommand(PTZ_UP_CONTROL);
	m_ptz_down.SetButtonCommand(PTZ_DOWN_CONTROL);
	m_ptz_left.SetButtonCommand(PTZ_LEFT_CONTROL);
	m_ptz_right.SetButtonCommand(PTZ_RIGHT_CONTROL);
	m_zoom_wide.SetButtonCommand(PTZ_ZOOM_DEC_CONTROL);
	m_zoom_tele.SetButtonCommand(PTZ_ZOOM_ADD_CONTROL);
	m_focus_near.SetButtonCommand(PTZ_FOCUS_DEC_CONTROL);
	m_focus_far.SetButtonCommand(PTZ_FOCUS_ADD_CONTROL);
	m_iris_open.SetButtonCommand(PTZ_APERTURE_ADD_CONTROL);
	m_iris_close.SetButtonCommand(PTZ_APERTURE_DEC_CONTROL);
	
	m_ptz_rightup.SetButtonCommand(EXTPTZ_RIGHTTOP);
	m_ptz_rightdown.SetButtonCommand(EXTPTZ_RIGHTDOWN);
	m_ptz_leftup.SetButtonCommand(EXTPTZ_LEFTTOP);
	m_ptz_leftdown.SetButtonCommand(EXTPTZ_LEFTDOWN);
...
void CPTZPannel::OnYwptzctrl() 
{
	((CNetSDKDemoDlg *)GetParent())->Ywptzctrl();
}
