...
void CUser_AddGroup::SetEnvrmt(USER_MANAGE_INFO *info, DeviceNode *dev)
{
	m_user_info = info;
	m_dev = dev;
}
...
void CUser_AddGroup::OnBtnOk() 
{
	...
	GetDlgItem(IDC_NAME_EDIT)->GetWindowText(ugInfo.name, USER_NAME_LENGTH);
	GetDlgItem(IDC_MEMO_EDIT)->GetWindowText(ugInfo.memo, MEMO_LENGTH);
