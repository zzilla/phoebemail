...
void CUser_ModifyGroup::SetEnvrmt(USER_MANAGE_INFO *info, DWORD gpIdx, DeviceNode *dev)
{
	m_dev = dev;
	m_user_info = info;
	m_gpIdx = gpIdx;
...
void CUser_ModifyGroup::OnBtnOk() 
{
	...
	GetDlgItem(IDC_GPNAME_NEW_EDIT)->GetWindowText(gpInfo.name, USER_NAME_LENGTH);
	GetDlgItem(IDC_GPMEMO_NEW_EDIT)->GetWindowText(gpInfo.memo, MEMO_LENGTH);
