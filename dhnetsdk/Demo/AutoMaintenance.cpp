	...
	: CDialog(CAutoMaintenance::IDD, pParent)
{
	...
	//}}AFX_DATA_INIT
	memset(&m_atC, 0, sizeof(DEV_AUTOMT_CFG));
	m_dev = NULL;
}
...
BOOL CAutoMaintenance::OnInitDialog() 
{
	...
	{
		DWORD ret = 0;
		BOOL b = CLIENT_GetDevConfig(m_dev->LoginID, DEV_AUTOMTCFG, 0, &m_atC, sizeof(DEV_AUTOMT_CFG), &ret, CONFIG_WAITTIME);
		if (!b || ret != sizeof(DEV_AUTOMT_CFG))
		{
...
void CAutoMaintenance::OnOK() 
{
	if (m_dev != NULL)
	{
		...
		BOOL b = CLIENT_SetDevConfig(m_dev->LoginID, DEV_AUTOMTCFG, 0, &m_atC, sizeof(m_atC), CONFIG_WAITTIME);
		if (!b)
		{
