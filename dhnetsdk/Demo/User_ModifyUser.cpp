...
BOOL CUser_ModifyUser::OnInitDialog() 
{
	...
	((CButton*)GetDlgItem(IDC_CHECK_REUSEABLE))->SetCheck(m_user_info->userList[m_userIdx].dwReusable);
	...
	((CButton*)GetDlgItem(IDC_CHECK_REUSEABLE2))->SetCheck(m_user_info->userList[m_userIdx].dwReusable);
...
void CUser_ModifyUser::SetEnvrmt(USER_MANAGE_INFO *info, DWORD userIdx, DeviceNode *dev)
{
	m_dev = dev;
	m_user_info = info;
	m_userIdx = userIdx;
...
void CUser_ModifyUser::OnBtnOk() 
{
	...
	GetDlgItem(IDC_NAME_NEW_EDIT)->GetWindowText(newInfo.name, USER_NAME_LENGTH);
	memcpy(newInfo.passWord, m_user_info->userList[m_userIdx].passWord, USER_PSW_LENGTH);
	GetDlgItem(IDC_MEMO_NEW_EDIT)->GetWindowText(newInfo.memo, MEMO_LENGTH);
