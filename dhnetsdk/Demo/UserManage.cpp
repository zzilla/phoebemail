...
{
	memset(&m_user_info, 0, sizeof(USER_MANAGE_INFO));
...
BOOL CUserManage::OnInitDialog() 
{
	...
	BOOL bRet = CLIENT_QueryUserInfo(m_dev->LoginID, &m_user_info, 1000);
	if (!bRet)
	{
...
{
	BOOL bRet = CLIENT_QueryUserInfo(m_dev->LoginID, &m_user_info, 10000);
	if (!bRet)
	{
		MessageBox(ConvertString("failed to get user info"));
	}
