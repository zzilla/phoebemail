...
//自定义全局函数
void g_SysTimetoYwtime(SYSTEMTIME *systime , NET_TIME *Ywtime)
{
	Ywtime->dwYear = (DWORD)systime->wYear;
	Ywtime->dwMonth = (DWORD)systime->wMonth;
	Ywtime->dwDay = (DWORD)systime->wDay;
	Ywtime->dwHour = (DWORD)systime->wHour;
	Ywtime->dwMinute = (DWORD)systime->wMinute;
	Ywtime->dwSecond = (DWORD)systime->wSecond;
}

//自定义全局函数
void g_YwtimetoSystime(NET_TIME *Ywtime , SYSTEMTIME *systime)
{
	systime->wYear = (WORD)Ywtime->dwYear;
	systime->wMonth = (WORD)Ywtime->dwMonth;
	systime->wDay = (WORD)Ywtime->dwDay;
	systime->wHour = (WORD)Ywtime->dwHour;
	systime->wMinute = (WORD)Ywtime->dwMinute;
	systime->wSecond = (WORD)Ywtime->dwSecond;
}

//自定义全局函数
CString g_TimeOutString(NET_TIME *ywtime )
{
	CString nStr;

	nStr.Format("%04d-%02d-%02d %02d:%02d:%02d", ywtime->dwYear,ywtime->dwMonth,ywtime->dwDay,
		ywtime->dwHour, ywtime->dwMinute, ywtime->dwSecond);
