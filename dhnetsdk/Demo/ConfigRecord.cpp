	...
	: CDialog(CConfigRecord::IDD, pParent)
{
	...
	//}}AFX_DATA_INIT
	m_pDev = 0;;
	memset(m_recCFG, 0, 16*sizeof(DEV_RECORD_CFG));
	m_bInited = FALSE;
	m_chnIdx = -1;
	m_weekIdx = -1;
}
...
void CConfigRecord::GetConfig()
{
	...
	BOOL bRet = FALSE;
	DWORD retlen = 0;
	bRet = CLIENT_GetDevConfig(m_pDev->LoginID, DEV_RECORDCFG, -1/*all channel*/,
		&m_recCFG, 16*sizeof(DEV_RECORD_CFG), &retlen, CONFIG_WAITTIME);
	if (!bRet || retlen != m_pDev->Info.byChanNum*sizeof(DEV_RECORD_CFG))
	{
...
void CConfigRecord::OnSelchangeComboChanno() 
{
	...
	for (i = 0; i < 6; i++)
	{
		...
		m_recCFG[m_chnIdx].stSect[weekIdx][i].iHourEnd = GetDlgItemInt(IDC_TIME_18+secIdx);
		secIdx++;
...
void CConfigRecord::OnSelchangeComboRecordweek() 
{
	...
	if (m_weekIdx >= 0)
	{
		...
		for (int i = 0; i < 6; i++)
		{
			...
			m_recCFG[m_chnIdx].stSect[m_weekIdx][i].iHourEnd = GetDlgItemInt(IDC_TIME_18+secIdx);
			secIdx++;
	...
	for (int i = 0; i < 6; i++)
	{
		...
		SetDlgItemInt(IDC_TIME_18+timeCtrlIdx, m_recCFG[m_chnIdx].stSect[weekIdx][i].iHourEnd);
		timeCtrlIdx++;
...
void CConfigRecord::OnApply() 
{
	...
	BOOL bRet = CLIENT_SetDevConfig(m_pDev->LoginID, DEV_RECORDCFG, -1/*all channel*/,
								&m_recCFG, 16*sizeof(DEV_RECORD_CFG), CONFIG_WAITTIME);
	if (!bRet)
	{
		((CNetSDKDemoDlg*)AfxGetMainWnd())->LastError();
		return;
