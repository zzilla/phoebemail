	...
	: CDialog(CDeviceWorkState::IDD, pParent)
{
	...
	memset(&m_diskState, 0 , sizeof(HARD_DISK_STATE));
}
...
BOOL CDeviceWorkState::OnInitDialog() 
{
	...
	BOOL bRet = CLIENT_QueryDevState(m_dev->LoginID, DEVSTATE_COMM_ALARM,
    ...
    for (int i=0; i<m_almState.alarminputcount; i++)
    {
	...
	bRet = CLIENT_QueryDevState(m_dev->LoginID, DEVSTATE_SHELTER_ALARM,
	...
	//record state
	bRet = CLIENT_QueryDevState(m_dev->LoginID, DEVSTATE_RECORDING,
	...
	bRet = CLIENT_QueryDevState(m_dev->LoginID, DEVSTATE_RESOURCE, (char *)dwResource, 3*sizeof(DWORD), &retlen);
	...
	bRet = CLIENT_QueryDevState(m_dev->LoginID, DEVSTATE_BITRATE, (char *)dwBitRate, channum*sizeof(DWORD), &retlen);
	...
	bRet = CLIENT_QueryDevState(m_dev->LoginID, DEVSTATE_CONN, (char *)dwNetConn, 11*sizeof(DWORD), &retlen);
	...
	bRet = CLIENT_QueryDevState(m_dev->LoginID, DEVSTATE_DISK,
								(char*)&m_diskState, sizeof(HARD_DISK_STATE), &retlen, 1000);
	if(!bRet || retlen != sizeof(HARD_DISK_STATE))
	{
		((CNetSDKDemoDlg *)GetParent())->LastError();//Peng Dongfeng 06.11.24
		return FALSE;
	}
