	...
	: CDialog(CConfigNetwork::IDD, pParent)
{
	...
	//}}AFX_DATA_INIT
	m_pDev = 0;
	memset(&m_netCFG, 0, sizeof(DEVICE_NET_CFG));
	memset(&m_sysCFG, 0, sizeof(DEV_SYSTEM_ATTR_CFG));
	//Begin: Add by li_deming(11517) 2008-1-30
	memset(&m_ddnsCFG,0,sizeof(DEV_MULTI_DDNS_CFG));
	//End:li_deming(11517)	
	m_bInited = FALSE;
	m_etherIdx = -1;
	m_hostIdx = -1;
...
void CConfigNetwork::GetConfig()
{
	...
	bRet = CLIENT_GetDevConfig(m_pDev->LoginID, DEV_DEVICECFG, 0,
								&m_sysCFG, sizeof(DEV_SYSTEM_ATTR_CFG), &retlen, CONFIG_WAITTIME*4);
	...
	bRet = CLIENT_GetDevConfig(m_pDev->LoginID, DEV_NETCFG, 0,
								&m_netCFG, sizeof(DEVICE_NET_CFG), &retlen, CONFIG_WAITTIME*4);
	if (!bRet || retlen != sizeof(DEVICE_NET_CFG))
	{
		((CNetSDKDemoDlg*)AfxGetMainWnd())->LastError();
		return;
	}
	...
	//DDNS config
	bRet = FALSE;
	bRet = CLIENT_GetDevConfig(m_pDev->LoginID, DEV_MULTI_DDNS, 0,
								&m_ddnsCFG, sizeof(DEV_MULTI_DDNS_CFG), &retlen, CONFIG_WAITTIME*4);
	selIndexOld=-1;	
	if(bRet == TRUE)
	{
		CString strItem="";
...
REMOTE_HOST_CFG *CConfigNetwork::GetRemoteHostType(int hostIdx, BOOL bChg)
{
	REMOTE_HOST_CFG *ret = 0;
	switch(hostIdx)
	{
	case 0:	/* 报警服务器 */
...
void CConfigNetwork::OnApply() 
{
	...
	GetDlgItemText(IDC_EDIT_DEVNAME, m_netCFG.sDevName, MAX_GENERAL_NAME_LEN);
	...
	//ethernet config
	GetDlgItemText(IDC_EDIT_NETIP, m_netCFG.stEtherNet[m_etherIdx].sDevIPAddr, MAX_IP_ADDRESS_LEN);
	GetDlgItemText(IDC_EDIT_NETIPMASK, m_netCFG.stEtherNet[m_etherIdx].sDevIPMask, MAX_IP_ADDRESS_LEN);
	GetDlgItemText(IDC_EDIT_NETGATEWAY, m_netCFG.stEtherNet[m_etherIdx].sGatewayIP, MAX_IP_ADDRESS_LEN);
	m_netCFG.stEtherNet[m_etherIdx].dwNetInterface = m_interfacesel.GetCurSel()+1;
	GetDlgItemText(IDC_EDIT_NETMAC, m_netCFG.stEtherNet[m_etherIdx].byMACAddr, MAC_ADDRESS_LEN);
	
	//remote config
	REMOTE_HOST_CFG *thishost = 0;
	...
	GetDlgItemText(IDC_EDIT_HOSTIP, thishost->sHostIPAddr, MAX_IP_ADDRESS_LEN);
	thishost->wHostPort = GetDlgItemInt(IDC_EDIT_HOSTPORT);
	GetDlgItemText(IDC_EDIT_HOSTUSERNAME, thishost->sHostUser, MAX_HOSTNAME_LEN);
	GetDlgItemText(IDC_EDIT_HOSTUSERPSW, thishost->sHostPassword, MAX_HOSTPSW_LEN);
	if (7 == m_hostIdx)
	{
		GetDlgItemText(IDC_EDIT_HOSTRESERVED, m_netCFG.sDdnsHostName, MAX_HOSTNAME_LEN);
	}
	
	//mail config
	GetDlgItemText(IDC_EDIT_MAILUSERNAME, m_netCFG.struMail.sUserName, MAX_GENERAL_NAME_LEN);
	GetDlgItemText(IDC_EDIT_MAILUSERPSW, m_netCFG.struMail.sUserPsw, MAX_GENERAL_NAME_LEN);
	GetDlgItemText(IDC_EDIT_MAILIP, m_netCFG.struMail.sMailIPAddr, MAX_IP_ADDRESS_LEN);
	m_netCFG.struMail.wMailPort = GetDlgItemInt(IDC_EDIT_MAILPORT);
	GetDlgItemText(IDC_EDIT_MAILDESADDR, m_netCFG.struMail.sDestAddr, MAX_MAIL_ADDRESS_LEN);
	GetDlgItemText(IDC_EDIT_MAILCCADDR, m_netCFG.struMail.sCcAddr, MAX_MAIL_ADDRESS_LEN);
	GetDlgItemText(IDC_EDIT_MAILBCCADDR, m_netCFG.struMail.sBccAddr, MAX_MAIL_ADDRESS_LEN);
	GetDlgItemText(IDC_EDIT_MAILSUBJECT, m_netCFG.struMail.sSubject, MAX_MAIL_ADDRESS_LEN);
	
//	strcpy(m_netCFG.struMail.sSenderAddr, "111111111111");
//
	//save
	BOOL bRet = CLIENT_SetDevConfig(m_pDev->LoginID, DEV_NETCFG, 0,
								&m_netCFG, sizeof(DEVICE_NET_CFG), CONFIG_WAITTIME);
	if (!bRet)
	{
		((CNetSDKDemoDlg*)AfxGetMainWnd())->LastError();				
		return;
	...
	else
	{
		...
		OnSelchangeComboDdnsid();
		bRet=CLIENT_SetDevConfig(m_pDev->LoginID,DEV_MULTI_DDNS,0,&m_ddnsCFG,sizeof(DEV_MULTI_DDNS_CFG),CONFIG_WAITTIME);
		if(bRet==TRUE)
		{
...
void CConfigNetwork::OnSelchangeComboRemotehost() 
{
	...
	if (m_hostIdx >= 0)
	{
		...
		GetDlgItemText(IDC_EDIT_HOSTIP, prehost->sHostIPAddr, MAX_IP_ADDRESS_LEN);
		prehost->wHostPort = GetDlgItemInt(IDC_EDIT_HOSTPORT);
		GetDlgItemText(IDC_EDIT_HOSTUSERNAME, prehost->sHostUser, MAX_HOSTNAME_LEN);
		GetDlgItemText(IDC_EDIT_HOSTUSERPSW, prehost->sHostPassword, MAX_HOSTPSW_LEN);
...
void CConfigNetwork::OnSelchangeComboEthernet() 
{
	...
	if (etherIdx < 0 || etherIdx >= MAX_ETHERNET_NUM) 
	{
		return;
	}
	m_etherIdx = etherIdx;
...
void CConfigNetwork::OnSelchangeComboDdnsid() 
{
	...
	{	
		...
		GetDlgItemText(IDC_EDIT_ALIAS,dns->szAlias,MAX_DDNS_ALIAS_LEN);
		GetDlgItemText(IDC_EDIT_DOMAINNAME,dns->szDomainName,MAX_DOMAIN_NAME_LEN);
		GetDlgItemText(IDC_EDIT_SERVERIP,dns->szServerIp,MAX_IP_ADDRESS_LEN);
		GetDlgItemText(IDC_EDIT_SERVRRTYPE,dns->szServerType,MAX_SERVER_TYPE_LEN);
