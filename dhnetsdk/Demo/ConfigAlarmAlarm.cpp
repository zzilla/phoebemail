	...
	: CDialog(CConfigAlarmAlarm::IDD, pParent)
{
	...
	//}}AFX_DATA_INIT
	m_pDev = 0;
	memset(m_alarmCFG, 0, MAX_ALARM_IN_NUM*sizeof(ALARM_IN_CFG));
	m_bReady = FALSE;
	m_almIdx = -1;
	m_weekIdx = -1;
	m_ptzchlIdx = -1;
...
void CConfigAlarmAlarm::SetAlarmInCFG(ALARM_IN_CFG *cfg)
{
	if (!cfg)
	{
		return;
	}
	memcpy(m_alarmCFG, cfg, MAX_ALARM_IN_NUM*sizeof(ALARM_IN_CFG));
...
void CConfigAlarmAlarm::OnSelchangeComboAlarmin() 
{
	...
	if (m_weekIdx >= 0)
	{
		...
		for (i = 0; i < 6; i++)
		{
			...
			m_alarmCFG[m_almIdx].stSect[m_weekIdx][i].iHourEnd = GetDlgItemInt(IDC_TIME_18+secIdx);
			secIdx++;
	...
	m_alarmCFG[m_almIdx].struHandle.dwActionFlag |= ((CButton*)GetDlgItem(IDC_CHECK_UPLOAD))->GetCheck()?ALARM_UPLOAD:0;
	m_alarmCFG[m_almIdx].struHandle.dwActionFlag |= ((CButton*)GetDlgItem(IDC_CHECK_LOCALTIP))->GetCheck()?ALARM_TIP:0;
	m_alarmCFG[m_almIdx].struHandle.dwActionFlag |= ((CButton*)GetDlgItem(IDC_CHECK_LOCALTOUR))->GetCheck()?ALARM_TOUR:0;
	m_alarmCFG[m_almIdx].struHandle.dwActionFlag |= ((CButton*)GetDlgItem(IDC_CHECK_SENDMAIL))->GetCheck()?ALARM_MAIL:0;
	m_alarmCFG[m_almIdx].struHandle.dwActionFlag |= ((CButton*)GetDlgItem(IDC_CHECK_PTZ))->GetCheck()?ALARM_PTZ:0;
	m_alarmCFG[m_almIdx].struHandle.dwActionFlag |= ((CButton*)GetDlgItem(IDC_CHECK_RECORD))->GetCheck()?ALARM_RECORD:0;
	m_alarmCFG[m_almIdx].struHandle.dwActionFlag |= ((CButton*)GetDlgItem(IDC_CHECK_OUTPUT))->GetCheck()?ALARM_OUT:0;
...
skipsave:
	...
	((CButton*)GetDlgItem(IDC_CHECK_UPLOAD))->SetCheck((m_alarmCFG[almIdx].struHandle.dwActionFlag&ALARM_UPLOAD)?1:0);
	((CButton*)GetDlgItem(IDC_CHECK_LOCALTIP))->SetCheck((m_alarmCFG[almIdx].struHandle.dwActionFlag&ALARM_TIP)?1:0);
	((CButton*)GetDlgItem(IDC_CHECK_LOCALTOUR))->SetCheck((m_alarmCFG[almIdx].struHandle.dwActionFlag&ALARM_TOUR)?1:0);
	((CButton*)GetDlgItem(IDC_CHECK_SENDMAIL))->SetCheck((m_alarmCFG[almIdx].struHandle.dwActionFlag&ALARM_MAIL)?1:0);
	((CButton*)GetDlgItem(IDC_CHECK_PTZ))->SetCheck((m_alarmCFG[almIdx].struHandle.dwActionFlag&ALARM_PTZ)?1:0);
	((CButton*)GetDlgItem(IDC_CHECK_RECORD))->SetCheck((m_alarmCFG[almIdx].struHandle.dwActionFlag&ALARM_RECORD)?1:0);
	((CButton*)GetDlgItem(IDC_CHECK_OUTPUT))->SetCheck((m_alarmCFG[almIdx].struHandle.dwActionFlag&ALARM_OUT)?1:0);
...
void CConfigAlarmAlarm::OnSelchangeComboAlarmweek() 
{
	...
	for (int i = 0; i < 6; i++)
	{
		...
		SetDlgItemInt(IDC_TIME_18+timeCtrlIdx, m_alarmCFG[m_almIdx].stSect[weekIdx][i].iHourEnd);
		timeCtrlIdx++;
...
void CConfigAlarmAlarm::GetAlarmInCFG(ALARM_IN_CFG *cfg)
{
	...
	memcpy(cfg, m_alarmCFG, MAX_ALARM_IN_NUM*sizeof(ALARM_IN_CFG));
}
