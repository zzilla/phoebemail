...
public:
	...
	void Lock(bool bReEnter = true);	//是否允许线程重复加锁
...
public:
	...
	virtual void UnRead(	);	
	virtual void Read(bool bReEnter = true); //是否允许线程重复加锁
	virtual void Write(bool bReEnter = true); //是否允许线程重复加锁
...
public:
	CReadWriteMutexLock( CReadWriteMutex& Mutex,
		bool bWrite = true,		//Write or Read Lock
		bool bAutoLock = true,	//if do lock in constructor
		bool bReEnter = true	//是否允许线程重复加锁
		);
